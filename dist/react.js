...
function hashAuthorization(parameters) {
  ...
  const hash = index.keccak256(index.concatHex([
    "0x05",
    index.toRlp([
  ...
  if (to === "bytes")
    return index.hexToBytes(hash);
  return hash;
}
async function recoverPublicKey({ hash, signature }) {
  const hashHex = index.isHex(hash) ? hash : index.toHex(hash);
  const { secp256k1 } = await Promise.resolve().then(() => require("./secp256k1-fyhWJSdd.js"));
  const signature_ = (() => {
...
async function verifyAuthorization({ address, authorization, signature }) {
  return index.isAddressEqual(index.getAddress(address), await recoverAuthorizationAddress({
    authorization,
    signature
  }));
}
...
function serializeErc6492Signature(parameters) {
  ...
  const signature_ = index.concatHex([
    index.encodeAbiParameters([{ type: "address" }, { type: "bytes" }, { type: "bytes" }], [address, data, signature]),
    erc6492MagicBytes
  ]);
  if (to === "hex")
    return signature_;
...
async function toSmartAccount(implementation) {
  ...
  return {
    ...
    async getNonce(parameters) {
      ...
      const nonce = await index.readContract(implementation.client, {
        abi: index.parseAbi([
    ...
    async isDeployed() {
      if (deployed)
        return true;
      const code = await index.getAction(implementation.client, getCode, "getCode")({
        address
      });
...
async function signMessage(client, { account: account_ = client.account, message }) {
  if (!account_)
    throw new index.AccountNotFoundError({
      docsPath: "/docs/actions/wallet/signMessage"
    });
  const account = index.parseAccount(account_);
...
async function signTypedData(client, parameters) {
  ...
  const account = index.parseAccount(account_);
  const types = {
    EIP712Domain: index.getTypesForEIP712Domain({ domain }),
    ...parameters.types
  };
  index.validateTypedData({ domain, message, primaryType, types });
  if (account.signTypedData)
    return account.signTypedData({ domain, message, primaryType, types });
  const typedData = index.serializeTypedData({ domain, message, primaryType, types });
  return client.request({
...
async function simulateContract(client, parameters) {
  ...
  const account = callRequest.account ? index.parseAccount(callRequest.account) : client.account;
  const calldata = index.encodeFunctionData({ abi, args, functionName });
  try {
    const { data } = await index.getAction(client, index.call, "call")({
      batch: false,
    ...
    const result = index.decodeFunctionResult({
      abi,
      args,
      functionName,
      data: data || "0x"
    ...
    throw index.getContractError(error, {
      abi,
      address,
      args,
...
async function signAuthorization(client, parameters) {
  ...
  if (!account.signAuthorization)
    throw new index.AccountTypeNotSupportedError({
      docsPath: "/docs/eip7702/signAuthorization",
      metaMessages: [
  ...
  const authorization = await index.prepareAuthorization(client, parameters);
...
const getAccountNonce = async (client, args) => {
  ...
  return await index.getAction(client, index.readContract, "readContract")({
...
const getSenderAddress = async (client, args) => {
  ...
  try {
    await index.getAction(client, simulateContract, "simulateContract")({
      address: entryPointAddress,
      abi: [
        {
          inputs: [
...
const isPluginInstalled = async (client, args) => {
  ...
  try {
    return await index.getAction(client, index.readContract, "readContract")({
      address,
      abi: index.KernelModuleIsModuleInstalledAbi,
...
async function signerTo7702Validator(client, { signer, entryPoint, kernelVersion }) {
  ...
  const chainId = client.chain?.id ?? await index.getChainId(client);
  const account = toAccount({
  ...
  return {
    ...
    async signUserOperation(userOperation) {
      const hash = index.getUserOperationHash({
        userOperation: {
          ...userOperation,
          signature: "0x"
        },
...
const encodeCallData$1 = async (calls, callType) => {
  ...
  const call = calls.length === 0 ? void 0 : calls[0];
  if (!call) {
    throw new Error("No calls to encode");
  }
  if (!callType || callType === "call") {
    return encodeExecuteSingleCall$1(call);
  }
  if (callType === "delegatecall") {
...
const getKernelV3Nonce = async (client, accountAddress) => {
  try {
    const nonce = await index.getAction(client, index.readContract, "readContract")({
      abi: index.KernelV3AccountAbi,
...
const getActionSelector = (entryPointVersion) => {
  if (entryPointVersion === "0.6") {
    ...
    return viem.toFunctionSelector(viem.getAbiItem({ abi: index.KernelV3AccountAbi, name: "execute" }));
  } else {
...
async function toKernelPluginManager(client, { sudo, regular, hook, pluginEnableSignature, validatorInitData, action, validAfter = 0, validUntil = 0, entryPoint, kernelVersion, chainId, isPreInstalled = false }) {
  ...
  const getSignatureData = async (accountAddress, selector, userOpSignature = "0x") => {
    ...
    if (regular) {
      ...
      const enableSignature = await getPluginEnableSignature(accountAddress);
      return index.getEncodedPluginsData({
        enableSignature,
        userOpSignature,
        action,
  ...
  const isPluginEnabled = async (accountAddress, selector) => {
    ...
    const isEnabled = await regular.isEnabled(accountAddress, action.selector) || await index.isPluginInitialized(client, accountAddress, regular.address);
    if (isEnabled) {
      pluginEnabled = true;
    }
    return isEnabled;
  ...
  const getPluginEnableSignature = async (accountAddress) => {
    ...
    if (!chainId) {
      chainId = client.chain?.id ?? await index.getChainId(client);
    }
    let ownerSig;
    if (entryPoint.version === "0.6") {
...
const encodeExecuteCall = (args, options, includeHooks = false) => {
  ...
  const executeUserOpSig = viem.toFunctionSelector(viem.getAbiItem({ abi: index.KernelV3ExecuteAbi, name: "executeUserOp" }));
  if (includeHooks) {
    return viem.concatHex([
      executeUserOpSig,
      viem.encodeFunctionData({
        abi: index.KernelV3ExecuteAbi,
        functionName: "execute",
...
const getPluginInstallCallData = (accountAddress, plugin) => {
  const data = viem.encodeFunctionData({
    abi: index.KernelModuleInstallAbi,
...
async function createKernelAccount(client, { plugins, entryPoint, index: index$1 = 0n, factoryAddress: _factoryAddress, accountImplementationAddress: _accountImplementationAddress, metaFactoryAddress: _metaFactoryAddress, address, kernelVersion, initConfig, useMetaFactory: _useMetaFactory = true, eip7702Auth, eip7702Account, pluginMigrations }) {
  ...
  const getMemoizedChainId = async () => {
    if (chainId)
      return chainId;
    chainId = client.chain ? client.chain.id : await index.getAction(client, index.getChainId, "getChainId")({});
    return chainId;
  };
...
const ecdsaGetMultiUserOpDummySignature = (userOperation, numOfUserOps, entryPoint, chainId) => {
  const userOpHash = index.getUserOperationHash({
    userOperation,
    entryPointAddress: entryPoint.address,
    entryPointVersion: entryPoint.version,
    chainId
  ...
  const merkleTree = new index.dist.MerkleTree(leaves, viem.keccak256, {
    sortPairs: true
  });
...
async function toMultiChainECDSAValidator(client, { signer, entryPoint, kernelVersion: _, validatorAddress: validatorAddress_, multiChainIds }) {
  ...
  const viemSigner = await toSigner({ signer });
  const chainId = await index.getChainId(client);
  const account = toAccount({
    address: viemSigner.address,
    async signMessage({ message }) {
      return signMessage(client, { account: viemSigner, message });
    ...
    async signTransaction(_2, __) {
      throw new index.SignTransactionNotSupportedBySmartAccountError();
    },
    async signTypedData(typedData) {
      return viemSigner.signTypedData(typedData);
    }
  ...
  const smartAccountQuery = reactQuery.useQuery({
    ...
    queryFn: async () => {
      ...
      return {
        ...
        account: {
          ...kernelAccount,
          createUserOp: index.createUserOps.bind(null, composeConfig, kernelAccount)
        }
      };
    },

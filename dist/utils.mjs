import { isUndefined as isUndefined$1, cloneDeepWith } from "lodash-es";
import { parseUnits, encodeFunctionData, formatUnits, isHex, zeroAddress, concatHex, toHex, hexToBytes, numberToHex } from "viem";
var commonjsGlobal = typeof globalThis !== "undefined" ? globalThis : typeof window !== "undefined" ? window : typeof global !== "undefined" ? global : typeof self !== "undefined" ? self : {};
function getDefaultExportFromCjs(x) {
  return x && x.__esModule && Object.prototype.hasOwnProperty.call(x, "default") ? x["default"] : x;
}
const bigintMax = (...args) => {
  return args.filter((x) => !isUndefined$1(x)).reduce((max, cur) => cur > max ? cur : max);
};
const bigintMin = (...args) => {
  return args.filter((x) => !isUndefined$1(x)).reduce((min, cur) => cur < min ? cur : min);
};
...
function toRpcUserOpCanonical(op) {
  const hx = (v) => typeof v === "string" && isHex(v) ? v : toHex(BigInt(v || "0"));
  const initCode = op.initCode && isHex(op.initCode) && op.initCode !== "0x" ? op.initCode : op.factory && op.factory !== zeroAddress && op.factoryData ? concatHex([
...
const hexify = (n) => n ? numberToHex(n) : void 0;
...
const createUserOp = async ({ account, chainId, calls }, config) => {
